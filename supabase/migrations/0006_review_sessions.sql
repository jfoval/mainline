-- Phase 2 · Slice 3 migration: weekly-review sessions.
--
-- One row per COMPLETED review (DATA-MODEL §review_sessions, trimmed to what the guided flow
-- needs). Rows are write-once on the client, so LWW has nothing to resolve here — the table
-- exists so "last reviewed" is the same fact on every device. Everything else the review reads
-- (inbox, projects, waiting, someday) is derived live from the tables 0003 already created.
--
-- Apply on top of 0003 (idempotent). Verify with scripts/verify-supabase.mjs.
...
create index if not exists review_sessions_user_seq_idx on public.review_sessions (user_id, server_seq);

drop trigger if exists review_sessions_stamp_seq on public.review_sessions;
create trigger review_sessions_stamp_seq before insert or update on public.review_sessions
...
create policy review_sessions_select on public.review_sessions for select using (user_id = auth.uid());
drop policy if exists review_sessions_insert on public.review_sessions;
create policy review_sessions_insert on public.review_sessions for insert with check (user_id = auth.uid());
drop policy if exists review_sessions_update on public.review_sessions;
create policy review_sessions_update on public.review_sessions for update using (user_id = auth.uid()) with check (user_id = auth.uid());
drop policy if exists review_sessions_delete on public.review_sessions;
create policy review_sessions_delete on public.review_sessions for delete using (user_id = auth.uid());

-- ─────────────────── sync_gtd, extended with the review_sessions branch ───────────────────
-- Full replacement of the 0003 function (same contract, one more table in push + pull).
