-- Phase 2 · Slice 4 migration: tickler / resurface dates on actions.
--
-- `resurface_on` is a DATE, not a timestamptz, on purpose: it's the user's local calendar day
-- ("bring this back on the 12th"), and pinning it to an instant would make the 12th arrive at
-- the wrong moment for anyone who travels. The client sends "YYYY-MM-DD"; nothing on the server
-- interprets it — no scheduler, no cron. An item is "back" simply because today's key is >= it.
--
-- Apply on top of 0003 (+0006). Idempotent. Verify with scripts/verify-supabase.mjs.

alter table public.actions add column if not exists resurface_on date;

-- ─────────────────── sync_gtd, extended with actions.resurface_on ───────────────────
-- Full replacement of the 0006 function (same contract, one more column on the actions branch).
