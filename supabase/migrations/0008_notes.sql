-- Phase 2 · Slice 5 migration: notes on actions and projects.
--
-- One plain text field per item — the thinking that hangs off a thing to do. It rides the same
-- whole-row LWW as everything else (a note edited on two devices resolves to the newer row;
-- notes are short and personal, so per-field merge would be more machinery than it's worth).
-- The someday → project promotion carries the note across client-side, so nothing here is
-- special-cased for it.
--
-- Apply on top of 0007. Idempotent. Verify with scripts/verify-supabase.mjs.
...
-- ─────────────────── sync_gtd, extended with the notes columns ───────────────────
-- Full replacement of the 0007 function (same contract, notes on the actions + projects branches).
